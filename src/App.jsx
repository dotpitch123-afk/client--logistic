...
//   //   { id: 'Box A', length: 0.0, width: 0.0, height: 0.0, quantity: 20, color: colorPalette[0].color, border: colorPalette[0].border },
//   //   { id: 'Box B', length: 0.0, width: 0.0, height: 0.0, quantity: 15, color: colorPalette[1].color, border: colorPalette[1].border },
//   //   { id: 'Box C', length: 0.0, width: 0.0, height: 0.0, quantity: 10, color: colorPalette[2].color, border: colorPalette[2].border },
//   //   { id: 'Box D', length: 0.0, width: 0.0, height: 0.0, quantity: 12, color: colorPalette[3].color, border: colorPalette[3].border }
//   // ]);

//   const [boxes, setBoxes] = useState([
//   { id: 'Box A', length: 0.0, width: 0.0, height: 0.0, quantity: 20, color: colorPalette[0].color, border: colorPalette[0].border, selected: true },
//   { id: 'Box B', length: 0.0, width: 0.0, height: 0.0, quantity: 15, color: colorPalette[1].color, border: colorPalette[1].border, selected: true },
//   { id: 'Box C', length: 0.0, width: 0.0, height: 0.0, quantity: 10, color: colorPalette[2].color, border: colorPalette[2].border, selected: true },
//   { id: 'Box D', length: 0.0, width: 0.0, height: 0.0, quantity: 12, color: colorPalette[3].color, border: colorPalette[3].border, selected: true }
// ]);
...
//   const boxTypeData = packingResult.box_summary.find(box => box.id === boxTypeId);
//   const boxConfig = boxes.find(b => b.id === boxTypeId);
...
//   // ctx.fillText(`Total Count: ${boxTypeData.count}`, 20, statsY);
//   // ctx.fillText(`Dimensions: ${boxTypeData.length}×${boxTypeData.width}×${boxTypeData.height}m`, 140, statsY);
...
//   // ctx.fillText(`Total Volume: ${totalVolume.toFixed(3)}m³`, 20, statsY + 15);
//   // ctx.fillText(`Container Fill: ${percentage}%`, 140, statsY + 15);
...
//         boxType: boxType.id,
//         boxIndex,
//         boxConfig: boxes.find(b => b.id === boxType.id)
//       });
//     });
//   });
...
// // 3. ADD THESE useEffect HOOKS FOR ANIMATION
...
//   try {
//     // Prepare all boxes for animation (same as startPackingAnimation)
//     const allBoxes = [];
//     packingResult.box_summary.forEach((boxType) => {
//       boxType.boxes.forEach((box, boxIndex) => {
//         allBoxes.push({
...
//     // Animate each box placement
//     for (let i = 0; i < allBoxes.length; i++) {
//       const box = allBoxes[i];
//       const color = box.boxConfig?.color || colorPalette[0].color;
//       const borderColor = box.boxConfig?.border || colorPalette[0].border;

//       // Create box with drop animation
//       const individualBoxGroup = new THREE.Group();
...
//             // Easing function for realistic drop
...
//             } else {
//               // Remove highlight after placement
//               setTimeout(() => {
//                 if (highlightMesh.parent) {
...
//     // Final camera sweep animation
//     const originalTheta = controlsRef.current.theta;
//     const sweepDuration = 3000; // 3 seconds
...
//         // Stop recording after final frames
//         setTimeout(() => {
//           mediaRecorder.stop();
...
//         const requiredColumns = ['id', 'length', 'width', 'height', 'quantity'];
...
//           const id = getColumnValue('id') || `Box ${String.fromCharCode(65 + index)}`;
...
//     ...boxes.map(box =>
//       `${box.id},${box.length},${box.width},${box.height},${box.quantity}`
//     )
//   ].join('\n');
...
//       <div className="font-medium mb-1">📋 Required CSV columns:</div>
//       <div className="font-mono text-xs">id, length, width, height, quantity</div>
//     </div> */}
//   </div>
...
//                         onChange={(e) => setShowBoxLabels(e.target.checked)}
//                         className="rounded"
//                       /> */}
//                       {/* <span className="text-sm text-gray-700">Box Labels</span> */}
//                     </label>
//                     <label className="flex items-center gap-2 cursor-pointer">
//                       {/* <input
//                         type="checkbox"
