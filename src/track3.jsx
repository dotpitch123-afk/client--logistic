...
const generateBoxTypeImage = useCallback((boxTypeId) => {
  ...
  boxTypeData.boxes.forEach((box, index) => {
    ...
    // Draw box number if box is large enough
...
const updateBox = (index, field, value, unit = null) => {
  const updatedBoxes = [...boxes];
  
  if (unit) {
    // Update unit for this box field
    updatedBoxes[index].units = {
      ...updatedBoxes[index].units,
      [field]: unit
    };
  ...
  if (field === 'color') {
    ...
    let convertedValue = Math.max(field === 'quantity' ? 1 : 0.01, parseFloat(value) || (field === 'quantity' ? 1 : 0.01));
    
    // Convert to base unit (meters for dimensions, kg for weight)
    if (['length', 'width', 'height'].includes(field)) {
      const currentUnit = updatedBoxes[index].units?.[field] || 'm';
      convertedValue = convertToMeters(convertedValue, currentUnit);
    } else if (field === 'weight') {
      const currentUnit = updatedBoxes[index].units?.weight || 'kg';
      convertedValue = convertWeight(convertedValue, currentUnit, 'kg');
    }
...
const updateContainer = (field, value, unit = null) => {
  if (unit) {
    // Just update unit (don’t change value)
...
// const updateContainer = (field, value, unit = null) => {
//   if (unit) {
//     // Only update unit and do not modify the container value
//     setContainerUnits(prev => ({
//       ...prev,
//       [field]: unit
//     }));
...
//   // When input changes, convert the entered value into base unit and store
//   let convertedValue = parseFloat(value) || 0;

//   if (['length', 'width', 'height'].includes(field)) {
//     convertedValue = convertToMeters(convertedValue, containerUnits[field]);
//   } else if (field === 'weight_capacity') {
//     convertedValue = convertWeight(convertedValue, containerUnits[field], 'kg');
//   }

//   setContainer(prev => ({
//     ...prev,
//     [field]: Math.max(field === 'weight_capacity' ? 0 : 0.01, convertedValue)
//   }));
// };
...
//   if (['length', 'width', 'height'].includes(field)) {
//     const converted = convertFromMeters(value, containerUnits[field]);
//     return converted.toFixed(containerUnits[field] === 'm' ? 2 : 0);
//   } else if (field === 'weight_capacity') {
//     const converted = convertWeight(value, 'kg', containerUnits[field]);
//     return converted.toFixed(containerUnits[field] === 'kg' ? 1 : 0);
//   }
...
{[
  { key: 'length', label: 'Length (L)', icon: '📏', units: ['m', 'cm', 'mm', 'ft', 'in'] },
  { key: 'width', label: 'Width (W)', icon: '📐', units: ['m', 'cm', 'mm', 'ft', 'in'] },
  { key: 'height', label: 'Height (H)', icon: '📊', units: ['m', 'cm', 'mm', 'ft', 'in'] },
  ...
  <div key={key}>
    ...
    <div className="flex gap-2">
      {/* <input
  key={`${key}-${containerUnits[key]}`}
  type="number"
  step={containerUnits[key] === 'm' || containerUnits[key] === 'kg' ? '0.1' : '1'}
...
/> */}
      {/* <select
        value={containerUnits[key]}
        onChange={(e) => updateContainer(key, getDisplayValue(key), e.target.value)}
        className="px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all bg-white min-w-[70px]"
      >
        {units.map(unit => (
      ...
      <input
  type="number"
  step={['m', 'kg'].includes(containerUnits[key]) ? '0.1' : '1'}
  value={getDisplayValue(key)}
  onChange={(e) => updateContainer(key, e.target.value)}
  className="flex-1 px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all"
...
>
  {units.map(unit => (
    ...
    <div className="text-xs text-gray-600 bg-white p-2 rounded border">
      📦 Volume: <strong>{(container.length * container.width * container.height).toFixed(2)}m³</strong>
      {useWeightConstraints && (
        <>
          <br/>
          ⚖️ Weight Limit: <strong>{container.weight_capacity.toFixed(1)}kg</strong>
        </>
      )}
      <div className="mt-1 text-xs text-gray-500">
        Display: {containerUnits.length}/{containerUnits.width}/{containerUnits.height}
        {useWeightConstraints && ` • ${containerUnits.weight_capacity}`}
      </div>
    </div>
  </div>
</div>
...
<div className={`text-xs text-gray-500 mt-2 bg-white p-2 rounded border flex justify-between ${!box.selected ? 'bg-gray-100 text-gray-400' : ''}`}>
  ...
  {useWeightConstraints && <span>⚖️ Wt: {box.weight}kg</span>}
...
{packingResult && (
 ...
 <div className="grid grid-cols-2 gap-3 mb-4">
      ...
      {/* {  weight user and total weight hidden when user choose by space option to store box inside container when use} */}
